...
from app.model import WebsiteBase, GroupWebsiteBase#, WAFGroupRuleBase
...
def process_violation(violation, session):
    ...
    if violation['audit_data']:
        for rule_violation in violation['audit_data']['messages']:
            ...
            # group_rule_name = get_group_rule_name_by_tag(list_tag, session)
            detail = {
...
def save_to_database(violation, rules, session):
    ...
    local_port = violation["transaction"]["remote_port"]
    status = violation["response"]["status"]
    print(status)
    ...
    #     print(website_domain, blocked_time, attacker_country, group_website, rule["system_action"],
    #           rule["group_rule"], violation_code)
    #     monitor_waf_obj = MonitorWAF(website_domain=website_domain,
    #                                  group_website=group_website,
    #                                  attacker_ip=attacker_ip,
    #                                  attacker_country=attacker_country,
    #                                  group_rule=rule["group_rule"],
...
def main():
    ...
    if isinstance(list_content, str) and list_content in 'error':
        ...
        for content in list_content:
            ...
            print("hihihi:   ",violation["response"]["status"])
            break
            # violation_header = violation["request"]["headers"]
